...
2. Register the client's intake: business type, target public function,
   prior filing history in Mexico if any.
3. Run the advisor in read-only mode against CompraNet, Mexico's federal government e-procurement system.
...
- named referral relationship with Mexican-licensed counsel or a
...
a working referral relationship with Mexican-licensed counsel or a
registered agent for whatever licensed representation the law of
Mexico requires for actual public-procurement filings.
