...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MEX, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up mx-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (mx-entity-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up rfc-unverified) ==")
    ...
    (println "== filing/submit eng-5 (rfc-unverified -> HARD hold) ==")
