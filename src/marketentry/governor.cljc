(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Mexico-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Mexico-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-mx-entity?
                                       true` (near-universal for Mexico
                                       ...
                                       Grounded in Mexican legal entity (RFC)
                                       typically required for federal
                                       procurement participation (contracting
                                       authorities / CompraNet).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(RFC/統一資格/CompraNet登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- mx-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-mx-entity? true`, INDEPENDENTLY verify
  `:has-mx-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-mx-entity?` ground truth (most Mexico public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-mx-entity? e))
                 (not (true? (:has-mx-entity? e))))
        [{:rule :mx-entity-missing
          :detail (str subject " はメキシコ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- rfc-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-rfc? true`, INDEPENDENTLY check
  `:rfc-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-rfc? e))
                 (not (true? (:rfc-verified? e))))
        [{:rule :rfc-unverified
          :detail (str subject " はRFC確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (mx-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (rfc-unverified-violations request st)
