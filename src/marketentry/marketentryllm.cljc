(ns marketentry.marketentryllm
  ...
  the MEX public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-mx-entity?=" (:has-mx-entity? e)
                        " rfc-verified?=" (:rfc-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-mx-entity? e))
                              (:has-mx-entity? e))
                          (or (not (:requires-rfc? e))
                              (:rfc-verified? e)))
                   0.9 0.3)}))
